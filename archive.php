<?php
/**
 * Category Archive — Research Journal filtered by category
 * Template: archive.php
 * Pepora Health — Conversion-optimised
 * Reuses the journal layout from home.php, filtered by category
 */
get_header();
...
// Reading time helper
if (!function_exists('pepora_reading_time')) {
...
<!-- ═══════════════════════════════════════════
     ARCHIVE — Research Journal by Category
     Pepora Health — Conversion-optimised
═══════════════════════════════════════════════ -->
  ...
  <div class="container">
    <div class="journal-hero-layout">
      <div class="journal-hero-text">
        ...
        <h1 class="journal-hero-title"><?php echo esc_html($cat_name ? $cat_name : 'Research Journal'); ?></h1>
        <p class="journal-hero-sub"><?php echo $cat_desc ? wp_kses_post($cat_desc) : 'Evidence-based insights into peptide science. No hype — only peer-reviewed data and compound analysis.'; ?></p>
      </div>
...
<section class="bg-gun2" style="padding:1.5rem 0;">
  <div class="container">
    <div class="journal-filters">
      <a href="<?php echo esc_url($blog_url); ?>" class="journal-pill">All Topics</a>
      <?php
      $categories = get_categories(array('hide_empty' => true));
      foreach ($categories as $cat) :
          $is_active = (is_category($cat->term_id)) ? ' active' : '';
      ?>
        <a href="<?php echo esc_url(get_category_link($cat->term_id)); ?>" class="journal-pill<?php echo $is_active; ?>"><?php echo esc_html($cat->name); ?></a>
      <?php endforeach; ?>
    </div>
  </div>
</section>
  ...
  <div class="container">
    ...
    <?php if ($wp_query->max_num_pages > 1) : ?>
      <div class="pagination" style="margin-top:2rem;">
        <?php if ($paged > 1) : ?>
          <a href="<?php echo esc_url(get_previous_posts_page_link()); ?>" class="btn btn-ghost">&larr; Previous</a>
        <?php endif; ?>
        <?php if ($paged < $wp_query->max_num_pages) : ?>
          <a href="<?php echo esc_url(get_next_posts_page_link($wp_query->max_num_pages)); ?>" class="btn btn-ghost">Next &rarr;</a>
        <?php endif; ?>
      </div>
    <?php endif; ?>
  </div>
    ...
    <div class="journal-cta-section">
      <div class="journal-cta-content">
        ...
        <div class="journal-cta-buttons">
          <a href="<?php echo esc_url($shop_url); ?>" class="btn btn-primary">Order Now — Ships Tomorrow</a>
